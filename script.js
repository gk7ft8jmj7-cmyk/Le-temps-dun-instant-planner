...
const ROLE_VIEWS={
  gerante:['dashboard','new-reservation','rachel','all-reservations','calendar','recap-rachel','settings'],
  menage:['rachel'],
  massage:['massage']
};
const ROLE_DEFAULT={gerante:'dashboard',menage:'rachel',massage:'massage'};
...
function isMassage(r){
  if(!r.options)return false;
  var opt=O.find(function(o){return o.name&&o.name.toLowerCase()==='massage';});
  if(opt&&r.options[opt.id])return true;
  if(r.options['o1'])return true;
  return false;
}
...
function init(){
  load(true);
  let n=0;
  function done(){if(++n>=5&&!ready){ready=true;load(false);go(defaultView());live();}}
...
// navigation
const VIEWS=['dashboard','new-reservation','rachel','massage','all-reservations','calendar','recap-rachel','settings'];
function go(v){
  const p=getProfile();
...
function fStat(){
  [['filter-status','<option value="">Tous les statuts</option>'],
   ...
   ['massage-filter-status','<option value="">Tous les statuts</option>'],
...
// ── VUE MASSAGE ────────────────────────────
const mSel=document.getElementById('massage-filter-status');
mSel.addEventListener('change',rMassage);

function massageCard(r){
  return '<div class="rachel-card" data-id="'+r.id+'">'+
    '<div class="rachel-card-header">'+
      '<div class="rachel-card-client">'+
        '<div class="rachel-client-name">'+x(fn(r))+(r.nbPersonnes&&r.nbPersonnes>1?' <span style="font-size:11px;color:var(--gray-400);">👥 '+r.nbPersonnes+' pers.</span>':'')+'</div>'+
      '</div>'+
      '<div class="rachel-card-status">'+bdg(r.statut)+'</div>'+
    '</div>'+
    '<div class="rachel-card-info">'+
...
function rMassage(){
  ...
  fStat();
  const sf=mSel.value;
  const c=document.getElementById('massage-cards');
  let list=[...R].filter(isMassage).sort((a,b)=>a.date<b.date?-1:1);
  if(sf)list=list.filter(r=>r.statut===sf);
  if(!list.length){c.innerHTML='<div class="empty-state"><div class="empty-state-icon">💆</div><p>Aucun massage prévu</p></div>';return;}
  c.innerHTML=list.map(r=>massageCard(r)).join('');
}
