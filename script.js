...
function showFermetureModal(){
  ...
  document.getElementById('ferm-save').addEventListener('click',function(){
    ...
    for(var ri=0;ri<R.length;ri++){
      ...
      // Hour check for single/partial day
      var rH=r.heure||'00:00';
      var rHD=r.heureDepart||'23:59';
      var fS2=heureDebut||'00:00';
      var fE2=heureFin||'23:59';
      // Check if any day overlaps
      var dCheck=new Date(Math.max(new Date(debut),new Date(r.date)));
      var dCheckEnd=new Date(Math.min(new Date(fin),new Date(rDep)));
      while(dCheck<=dCheckEnd){
        var ds3=dCheck.toISOString().split('T')[0];
        var rS3=ds3===r.date?rH:'00:00';
        var rE3=ds3===rDep?rHD:'23:59';
        var fS3=ds3===debut?fS2:'00:00';
        var fE3=ds3===fin?fE2:'23:59';
        if(rS3<fE3&&rE3>fS3){conflict=r;break;}
    ...
    if(conflict){
      ...
      if(conflict.heure)msg2+=' à partir de '+conflict.heure;
...
function rCal(){
  ...
  // Add fermeture button if not exists
  if(!document.getElementById('btn-fermer-chambre')){
    var calHeader = document.querySelector('#view-calendar .view-header .cal-nav');
    if(calHeader){
      var fb=document.createElement('button');
      fb.id='btn-fermer-chambre';
      fb.className='btn btn-danger btn-sm';
      fb.style.marginLeft='12px';
      fb.innerHTML='🔴 Fermer chambre';
      fb.addEventListener('click',showFermetureModal);
      calHeader.parentNode.appendChild(fb);
    }
  }
  ...
  for(let d=1;d<=dm;d++){
    ...
    var fermStyle='';
    if(ft){
      if(ft.type==='full') fermStyle='background:#B83030;';
      else if(ft.type==='matin') fermStyle='background:linear-gradient(to bottom,rgba(184,48,48,0.5) 50%,transparent 50%);';
      else if(ft.type==='soir') fermStyle='background:linear-gradient(to bottom,transparent 50%,rgba(184,48,48,0.5) 50%);';
    }
    var fermLabel='';
    if(ft){
      var fHD=ft.f&&ft.f.heureDebut?ft.f.heureDebut:'';
      var fHF=ft.f&&ft.f.heureFin?ft.f.heureFin:'';
      var heuresLabel=fHD&&fHF?' ('+fHD+'-'+fHF+')':'';
      fermLabel='<div class="cal-ferme-label">🔴 '+x(ft.motif||'Fermé')+heuresLabel+'</div>';
    }
  ...
  g.querySelectorAll('.cal-cell[data-date]').forEach(c=>c.addEventListener('click',function(){
    ...
    if(ftClick){
      ...
      dl.innerHTML='<div style="padding:12px;background:var(--red-bg);border-radius:var(--radius-md);border:1px solid var(--red);">'+
        '<div style="font-size:16px;font-weight:700;color:var(--red);">🔴 Chambre fermée</div>'+
        '<div style="font-size:14px;color:var(--gray-700);margin-top:4px;">Motif : '+(f?x(f.motif):'—')+'</div>'+
        '<div style="font-size:13px;color:var(--gray-400);margin-top:2px;">Du '+(f?fd(f.dateDebut):'—')+' au '+(f?fd(f.dateFin):'—')+'</div>'+
        '<button class="btn btn-danger" style="margin-top:12px;" id="btn-remove-ferm">Ouvrir la chambre</button>'+
      '</div>';
