...
function rRecap(){
  db.ref('rachel_historique').once('value').then(snap=>{
    ...
    // LOGIQUE SIMPLE :
    // - Paiements actifs (non annulés) : leur montant est déduit du total dû
    // - Paiements annulés : leur montant EST dû (comme si pas payé), mais reste visible barré
    // - Paiements supprimés : n'existent plus, ni dans le dû ni dans l'historique
    
    // Dernière date de paiement actif (pour savoir quelles réservations sont dans la période)
    const activeHist = hist.filter(h => !h.annule);
    const last = activeHist.length ? activeHist[activeHist.length-1].timestamp : null;
    
    // Heures dues depuis le dernier paiement actif
    ...
    // Montant total dû = heures dues (les paiements actifs ont déjà "réinitialisé" la période)
    // On doit aussi ajouter les montants des paiements annulés qui sont dans la période actuelle
    const montantAnnulesEnCours = hist.filter(h => h.annule && (!last || h.timestamp > last))
      .reduce((acc, h) => acc + (h.montant || 0), 0);
    
    const netDu = Math.round((heuresDues + montantAnnulesEnCours) * 100) / 100;
    const mt = Math.abs(netDu);
    ...
    // Montant dû simple - rouge si négatif (crédit)
    document.getElementById('recap-total-heures').textContent = fh(tH);
    document.getElementById('recap-total-montant').textContent = netDu.toFixed(2)+' €';
    document.getElementById('recap-total-montant').style.color = netDu <= 0 ? 'var(--green)' : 'inherit';
    window._rc = {tH, mt, netDu, last};
...
document.getElementById('btn-payer-rachel').addEventListener('click',()=>{
  ...
  const entry={timestamp:now,heures:d.tH,montantDu:d.netDu||d.mt,montant:montantPaye,credit:credit,periode:debut+' → '+fin};
