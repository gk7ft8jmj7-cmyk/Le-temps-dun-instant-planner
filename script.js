...
const ROLE_VIEWS={
  gerante:['dashboard','new-reservation','menage','massage','all-reservations','calendar','temps-travail','settings'],
  menage:['dashboard','menage','calendar'],
...
function applyRole(){
  ...
  // libellés dynamiques
  const navMenage=document.querySelector('.nav-item[data-view="menage"] span');
  if(navMenage)navMenage.textContent=p.role==='menage'?('Mes tâches — '+p.name):'Vue Ménage';
...
// navigation
const VIEWS=['dashboard','new-reservation','menage','massage','all-reservations','calendar','temps-travail','settings'];
function go(v){
  const p=getProfile();
...
function rDash(){
  ...
  c.innerHTML=list.map(r=>{
    ...
    const badges=(r.rachel?'<span class="rachel-badge">🧹 Rachel</span>':'')+(r.milla?'<span class="rachel-badge">🧹 Milla</span>':'');
    return '<div class="resa-row'+((r.rachel||r.milla)?' resa-row-rachel':'')+'">'+ 
      '<div class="resa-row-left">'+
        '<div class="resa-row-name">'+(isBooking(r)?'<span style="color:#1A5F9E;font-weight:700;">':'')+x(fn(r))+(isBooking(r)?'</span>':'')+(r.nbPersonnes&&r.nbPersonnes>1?' <span style="font-size:11px;color:var(--gray-400);">👥 '+r.nbPersonnes+' pers.</span>':'')+' '+badges+'</div>'+
  ...
  return '<div class="rachel-card'+(highlighted?' rachel-card-highlighted':'')+'" data-id="'+r.id+'">'+
    '<div class="rachel-card-header">'+
      '<div class="rachel-card-client">'+
        '<div class="rachel-client-name">'+(isBooking(r)?'<span style="color:#1A5F9E;font-weight:700;">':'')+x(fn(r))+(isBooking(r)?'</span>':'')+(r.nbPersonnes&&r.nbPersonnes>1?' <span style="font-size:11px;color:var(--gray-400);">👥 '+r.nbPersonnes+' pers.</span>':'')+' '+badges+'</div>'+
...
// ── VUE MÉNAGE (Rachel / Milla) ────────────
const rSel=document.getElementById('rachel-filter-status');
rSel.addEventListener('change',rMenage);
document.querySelectorAll('#menage-tabs .tab-btn').forEach(b=>b.addEventListener('click',()=>{
  document.querySelectorAll('#menage-tabs .tab-btn').forEach(x=>x.classList.remove('active'));
  b.classList.add('active');rMenage();
}));

function rMenage(){
  const p=getProfile();
  const pid=(p&&p.role==='menage')?p.id:null;
  if(!rSel.dataset.init){rSel.dataset.init='1';}
  fStat();
  const sf=rSel.value;
  const tabBtn=document.querySelector('#menage-tabs .tab-btn.active');
  const tabVal=tabBtn?tabBtn.dataset.tab:'avenir';
  const today=new Date().toISOString().split('T')[0];
  const title=document.getElementById('menage-view-title');
  if(title)title.textContent=pid?('Mes tâches — '+labelPerson(pid)):'Vue Ménage';
  const c=document.getElementById('rachel-cards');
  let list=[...R].filter(r=>pid?r[pid]:(r.rachel||r.milla));
...
function massageGeranteCard(m){
  ...
  return '<div class="rachel-card" data-id="'+m.id+'">'+
    '<div class="rachel-card-header">'+
      '<div class="rachel-card-client"><div class="rachel-client-name">📅 '+fd(m.date)+' · '+x(m.heureDebut||'')+'</div>'+
...
function massageStaffCard(m,pid){
  ...
  return '<div class="rachel-card massage-card" data-id="'+m.id+'">'+
    '<div class="rachel-card-header">'+
      '<div class="rachel-card-client"><div class="rachel-client-name">📅 '+fd(m.date)+' · '+x(m.heureDebut||'')+'</div></div>'+
    '</div>'+
    '<div class="rachel-card-info">'+
...
document.getElementById('massage-form').addEventListener('submit',function(e){
  ...
  if(!date||!heureDebut||!Object.keys(assignees).length){toast('Date, heure et au moins une personne assignée sont requis.','error');return;}
  const data={date,heureDebut,nbPersonnes,assignees,notes:document.getElementById('mf-notes').value.trim(),createdAt:mfEditId?(M.find(i=>i.id===mfEditId)||{}).createdAt||Date.now():Date.now()};
...
function cDetail(ds){
  ...
  if(dmg.length){
    html+=dmg.map(m=>{
      ...
      return '<div class="resa-row" style="margin-bottom:8px;background:rgba(90,140,90,0.08);"><div class="resa-row-left"><div class="resa-row-name">💆 Massage · '+(m.nbPersonnes||1)+' pers.</div><div class="resa-row-sub">🕒 '+(m.heureDebut||'—')+' · '+x(names)+'</div></div></div>';
    }).join('');
  }
