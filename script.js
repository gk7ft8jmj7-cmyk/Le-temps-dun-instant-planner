...
function showFermetureModal(){
  ...
  m.innerHTML = '<div class="modal" style="max-width:420px;">'+
    ...
    '<div class="form-group" style="margin-bottom:12px;">'+
      ...
      '<input type="date" id="ferm-debut" style="height:40px;width:100%;padding:0 12px;border:1px solid var(--gray-200);border-radius:var(--radius-md);" />'+
    '</div>'+
    '<div class="form-group" style="margin-bottom:12px;">'+
      '<label>Date de fin</label>'+
      '<input type="date" id="ferm-fin" style="height:40px;width:100%;padding:0 12px;border:1px solid var(--gray-200);border-radius:var(--radius-md);" />'+
    '</div>'+
  ...
  document.getElementById('ferm-save').addEventListener('click',function(){
    ...
    // Check no reservation on those dates
    var conflict = R.find(function(r){
      return r.date >= debut && r.date <= fin;
    });
    if(conflict){toast('Impossible — une réservation existe sur cette période ('+x(fn(conflict))+').','error');return;}

    db.ref('fermetures').push({dateDebut:debut, dateFin:fin, motif:motif||'Fermé', createdAt:Date.now()})
      .then(function(){toast('Chambre fermée du '+fd(debut)+' au '+fd(fin),'success');m.remove();rCal();})
...
function rCal(){
  ...
  g.querySelectorAll('.cal-cell[data-date]').forEach(c=>c.addEventListener('click',function(){
    if(c.dataset.ferme==='1'){
      var f=getFermeture(c.dataset.date);
      // Show fermeture detail with option to remove
          ...
          db.ref('fermetures').once('value',function(snap){
            ...
            Object.entries(snap.val()).forEach(function(e){
              if(e[1].dateDebut===f.dateDebut&&e[1].dateFin===f.dateFin){
                db.ref('fermetures/'+e[0]).remove().then(function(){toast('Chambre ouverte ✓','success');rCal();document.getElementById('cal-detail').style.display='none';});
              }
            });
          });
        }
